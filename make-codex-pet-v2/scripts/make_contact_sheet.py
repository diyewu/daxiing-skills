...
def main() -> None:
    ...
    parser.add_argument("--look-output")
    ...
    if args.look_output:
        look_output = Path(args.look_output).expanduser().resolve()
        look_output.parent.mkdir(parents=True, exist_ok=True)
        look_top = 9 * (cell_h + LABEL_HEIGHT)
        sheet.crop((0, look_top, width, height)).save(look_output)
        print(f"wrote {look_output}")
